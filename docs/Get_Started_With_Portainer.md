...
What is wrong with installing softwares to your Laptop or Desktop? Sooner or later you will end up in having a lot of unnecessary software on your machine. Or you have to install various software with various dependencies which could not be resolved. There are so many possibilities to separate a test environment from your host machine:
...
All of them has the advantages and disadvantages. Proxmox is really good if you have a separate hardware to install. It can handle LXC Containers and HW virtualisation as well. VirtualBox is an easy-to-use Virtualisation platform. It can be installed on Linux, Windows and even on MAC.  
...
It is not an elegant to publish your Portainer to the public Internet. Of Course this not an issue when you install portainer inside a private network. But if you plainning to use portainer on a VPS which has public IP address it is recommended not to bind the bublic IP address, or set up a firewall (iptables).
...
```bash
root@vps10:~# docker ps --filter name=portainer
...
![DeployRegistry](assets/images1634214698.jpg)
...
* Publish all exposed network ports to random host ports: OFF!
* Publish Port 18081 --> 80

!!! important
    Always pull image: off --> "An anonymous account  will be limited to 100 pulls every 6 hours." But If you wish you can keep it in On.
...
!!! warning
    Since we turned of the "Always pull the image" option, the image have to be manually pulled: `docker pull joxit/docker-registry-ui`

    Or you can use the Portainer Image menu for pulling the image.
...
In this case the REGISTRY_URL shuld be `http://10.5.0.2:18081`.
...
/usr/share/nginx/html # curl -i http://registry:5000
HTTP/1.1 200 OK
...
Regarding the port, inside a Docker network always use the port on which the process itself is listening, NOT the published port. (`0.0.0.0:49153->5000/tcp`)
...
This is likely becase the `REGISTRY_URL` is misconfigured.
...
* If you accidetally delete the container you loose all it's configuration (port mapping, environments, voulume and network settings).
